...
using DownloadStatus = DSWS140::BusinessObjects.DSWS.BIPlatform.DownloadStatus;
using PromptValue = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.PromptValue;
using PromptValueR = DSWS140::BusinessObjects.DSWS.ReportEngine.PromptValue;
using DiscretePromptValue = DSWS140::BusinessObjects.DSWS.ReportEngine.DiscretePromptValue;
using RangePromptValue = DSWS140::BusinessObjects.DSWS.ReportEngine.RangePromptValue;
using DiskUnmanaged = DSWS140::BusinessObjects.DSWS.BIPlatform.Dest.DiskUnmanaged;
...
using RetrievePromptsInfo = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrievePromptsInfo;
using RetrieveReportList = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrieveReportList;
using Action = DSWS140::BusinessObjects.DSWS.ReportEngine.Action;
using Close = DSWS140::BusinessObjects.DSWS.ReportEngine.Close;
using PromptInfo = DSWS140::BusinessObjects.DSWS.ReportEngine.PromptInfo;
using DocumentInformation = DSWS140::BusinessObjects.DSWS.ReportEngine.DocumentInformation;
using RetrieveMustFillInfo = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrieveMustFillInfo;
using RetrieveViewSupport = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrieveViewSupport;
using Report_ = DSWS140::BusinessObjects.DSWS.ReportEngine.Report;
using XMLView = DSWS140::BusinessObjects.DSWS.ReportEngine.XMLView;
using CharacterView = DSWS140::BusinessObjects.DSWS.ReportEngine.CharacterView;
using RetrieveData = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrieveData;
using RetrieveView = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrieveView;
using OutputFormatType = DSWS140::BusinessObjects.DSWS.ReportEngine.OutputFormatType;
using ViewModeType = DSWS140::BusinessObjects.DSWS.ReportEngine.ViewModeType;
using NavigateToPath = DSWS140::BusinessObjects.DSWS.ReportEngine.NavigateToPath;
using ViewType = DSWS140::BusinessObjects.DSWS.ReportEngine.ViewType;
using ViewSupport = DSWS140::BusinessObjects.DSWS.ReportEngine.ViewSupport;
using RetrieveXMLView = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrieveXMLView;
using RetrieveBinaryView = DSWS140::BusinessObjects.DSWS.ReportEngine.RetrieveBinaryView;
using BinaryView = DSWS140::BusinessObjects.DSWS.ReportEngine.BinaryView;
...
using WebiPrompt = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.WebiPrompt;
using WebiFormatEnum = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.WebiFormatEnum;
using WebiFormatOptions = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.WebiFormatOptions;
...
using CrystalReportFormatOptions = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.CrystalReportFormatOptions;
using DataProvider = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.DataProvider;
using ScheduleTypeEnum = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.ScheduleTypeEnum;
using SchedulingInfo = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.SchedulingInfo;
using ReportFormatEnum = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.ReportFormatEnum;
using Destination = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.Destination;
using Program = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.Program;
using Smtp = DSWS140::BusinessObjects.DSWS.BIPlatform.Dest.Smtp;
using Notifications = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.Notifications;
using ReportParameter = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.ReportParameter;
using DeletionCriteria = DSWS140::BusinessObjects.DSWS.BIPlatform.DeletionCriteria;
using Publication = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.Publication;
using FormatTypeEnum = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.FormatTypeEnum;
using OutputFormat = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.OutputFormat;
using ScheduleStatusEnum = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.ScheduleStatusEnum;
using ScheduleOutcomeEnum = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.ScheduleOutcomeEnum;
using File = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.File;
using Path = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.Path;
using FileProperties = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.FileProperties;
...
using CurrentValues = DSWS140::BusinessObjects.DSWS.BIPlatform.Desktop.CurrentValues;
using FileOptions = DSWS140::BusinessObjects.DSWS.BIPlatform.FileOptions;
using BusinessObjectsUtils.bo_session;

namespace BusinessObjectsUtils.bo_scheduler.dsws
{
    public class Scheduler140 : IScheduler
	{
        ...
        public Scheduler140(SessionManager sessionmanager)
        {
            if (!(sessionmanager.Session is Session140)) throw new Exception("Wrong version between session and scheduler!");
            _sessionmanager = sessionmanager;
            _biplatform = (sessionmanager.Session as Session140).BIPlatform;
            _reportengine = (sessionmanager.Session as Session140).ReportEngine;
        }
                ...
                for (int p=0; p < nbPages; p++){
                    ...
                    for(var r=0; r < nbReports; r++){
					    ...
					    try{
						    ...
						    }else if(boInfoObjectPrompts is Publication){
                                var reportPublication = (Publication)boInfoObjectPrompts;
						    }
					    }catch(Exception ex){
        ...
        public ExitCode ScheduleReport(ref Report[] pReports, bool pSetPrompts, DateTime pDate, string pPath, string pOutFormat, bool pWaitEnd, bool pCleanup, string pNotifEmail){
            ...
            if (!string.IsNullOrEmpty(pPath) ){
                lDestinationDisk = new[]{ new Destination{
                    ...
                    DestinationScheduleOptions = new DiskUnmanagedScheduleOptions { DestinationFiles = new [] { pPath } }
                }};
            }else{
                lDestinationDisk = new[]{ new Destination{
                ...
                try{
                    ...
                    switch(boInfoObject.Kind){
					    ...
					    case "CrystalReport":
                            ...
                            break;
					    case "Publication":
                            var reportPublication = (Publication)boInfoObject;
						    FormatTypeEnum publicationFormat;
						    switch(pOutFormat){
                                case "excel": publicationFormat = FormatTypeEnum.EXCEL; break;
                                case "pdf": publicationFormat = FormatTypeEnum.PDF; break;
                                default: publicationFormat = FormatTypeEnum.FULLCLIENT; break;
						    }
                            reportPublication.OutputFormat = new OutputFormat{  Format = publicationFormat, FormatSpecified = true };
                            //outcome = reportPublication.SchedulingInfo.Outcome;
                            break;
					    default:
                    ...
                    if(boInfoObjects!=null && boInfoObjects.InfoObject!=null && boInfoObjects.InfoObject.Length != 0){
                        ...
                        switch (pFormat){
                            case "DEFAULT":
                                ...
                                var downloadId = _biplatform.StartSingleDownload(boInfoObject.CUID, 0, fileOptions);
        ...
        private File GetFile(InfoObject pInfoObject){
            ...
            switch (pInfoObject.Kind){
                ...
                case "Publication": fileProperties = ((Publication)pInfoObject).FileProperties; break;
        ...
        private SchedulingInfo GetSchedulingInfo(InfoObject pInfoObject){
            ...
            switch (pInfoObject.Kind){
                ...
                case "Publication": schedulingInfo = ((Publication)pInfoObject).SchedulingInfo; break;
